...
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
General job submitter, CRYSTAL parallel properties version, for Imperial HPC - Setting up
...
================================================================================
    ...
    Please specify your installation path (by default ${HOME}/runCRYSTAL-Pprop/):
...
EOF
    ...
    if [[ -z ${SCRIPTDIR} ]]; then
        SCRIPTDIR=${HOME}/runCRYSTAL-Pprop/
    fi
...
function set_exe {
    cat << EOF
================================================================================
    Please specify the directory of properties exectuable of CRYSTAL, 
...
EOF
    ...
    cat << EOF
================================================================================
    Please specify the properties exectuable of CRYSTAL package (by default Pproperties):
...
# Setup default file format tables
    ...
    sed -i "${LINE_PRE}i[jobname].d3           [jobname].d3           CRYSTAL-Pprop main input file" ${SETFILE}
    sed -i "${LINE_PRE}i[pre_job].f9           fort.20                Last step wavefunction of the previous SCF run" ${SETFILE}
    ...
    sed -i "${LINE_POST}i[jobname].GRED         GRED.DAT             Real space information for cryapi_inp" ${SETFILE}
    sed -i "${LINE_POST}i[jobname].KRED         KRED.DAT             k space information for cryapi_inp" ${SETFILE}
    sed -i "${LINE_POST}i[jobname].DIEL         DIEL.DAT             Dielectric constants" ${SETFILE}
    sed -i "${LINE_POST}i[jobname].f25          fort.25              All Crgra2006 format data " ${SETFILE}
    sed -i "${LINE_POST}i[jobname].BAND         BAND.DAT             Band xmgrace format " ${SETFILE}
    sed -i "${LINE_POST}i[jobname].DOSS         DOSS.DAT             Dos xmgrace format" ${SETFILE}
    sed -i "${LINE_POST}i[jobname].prop3d       fort.31              All 3D grid data" ${SETFILE}
    sed -i "${LINE_POST}i[jobname]_CHG.CUBE     DENS_CUBE.DAT        3D charge density Gaussian CUBE format" ${SETFILE}
    sed -i "${LINE_POST}i[jobname].RHOLINE      RHOLINE.DAT          1D charge density and gradient" ${SETFILE}
    sed -i "${LINE_POST}i[jobname]_SPIN.CUBE    SPIN_CUBE.DAT        3D spin density CUBE format" ${SETFILE}
    sed -i "${LINE_POST}i[jobname]_POT.CUBE     POT_CUBE.DAT         3D electrostatic potential CUBE format" ${SETFILE}
    sed -i "${LINE_POST}i[jobname].POTC         POTC.DAT             Electrostatic potential and derivatives" ${SETFILE}
...
function set_commands {
    bgline=`grep -nw "# >>> CRYSTAL-Pprop job submitter settings >>>" ${HOME}/.bashrc`
    edline=`grep -nw "# <<< finish CRYSTAL-Pprop job submitter settings <<<" ${HOME}/.bashrc`
    ...
    echo "# >>> CRYSTAL-Pprop job submitter settings >>>" >> ${HOME}/.bashrc
    echo "alias Pprop='${SCRIPTDIR}/gen_sub'" >> ${HOME}/.bashrc
    echo "alias setpprop='cat ${SCRIPTDIR}/settings'" >> ${HOME}/.bashrc
    ...
    echo "# <<< finish CRYSTAL-Pprop job submitter settings <<<" >> ${HOME}/.bashrc
...
================================================================================
    ...
    Pprop - executing parallel properties calculations of CRYSTAL

        Pprop -in <input> -wt <walltime> -nd <node> -- -<other options>
    ...
    setpprop - print the file 'settings'
