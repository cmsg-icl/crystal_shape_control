...
do
  ...
  echo "TITLE,TOT E(HARTREE),N SCF CYC,TIME PER CYC(S)" >> ../energies.csv #write energies to root directory
  echo "$rt_dir,$energy,${n_scf},${time_cyc}" >> ../energies.csv #write energies to root directory
  echo "TITLE,GRAD(HARTREE/BOHR),N SCF CYC,TIME PER CYC(S)" >> ../forces.csv #write forces to root directory
